...
export function CollapsibleExplanationTable(props: CollapsibleExplanationTableProps) {
  return (
    <Paper style={{ marginTop: '32px', padding: '16px', border: '1px solid #f0f0f0', backgroundColor: '#ffffff' }}>
      ...
      >
        ...
        <ChevronDown
          styles={{
            ...
            transform: props.opened ? 'rotate(180deg)' : 'rotate(0deg)',
            fontWeight: 'bold',
            fontSize: '18px',
          }}
        />
      ...
      <Collapse in={props.opened}>
        <Table highlightOnHover mt="md" style={{ borderCollapse: 'collapse', textAlign: 'center' }}>
          ...
          <tbody>
            {props.cronExamples.map((example, index) => (
              <tr key={index}>
