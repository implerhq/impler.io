...
export class ProjectInvitation {
  ...
  async exec(command: ProjectInvitationCommand) {
    const invitation = await this.projectInvitationRepository.create({
      invitationEmails: command.invitationEmails,
    ...
    if (this.emailService.isConnected()) {
      ...
      command.invitationEmails.forEach((invitationEmail) => {
        console.log(
          invitationEmail,
          EMAIL_SUBJECT.PROJECT_INVITATION,
          emailContents,
          command.invitatedBy,
          command.userName
        );
      ...
      command.invitationEmails.forEach(async (invitationEmail) => {
